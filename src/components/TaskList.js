...
const TaskList = () => {
  ...
  useEffect(() => {
    const fetchTasks = async () => {
      try {
        const response = await axios.get('http://localhost:5000/api/tasks'); // Adjust URL as per your backend
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`http://localhost:5000/api/tasks/${id}`); // Adjust URL as per your backend
  ...
  const handleToggleStatus = async (id, currentStatus) => {
    ...
    try {
      await axios.put(`http://localhost:5000/api/tasks/${id}`, { status: newStatus }); // Adjust URL as per your backend
